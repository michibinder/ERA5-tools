...
def prepare_interpolated_ml_ds(ds,ds_T21,ml_coeff,ds_interp_path):
    """Inpterpolate the model level dataset to a regular grid and combine with T21 (filtered) dataset"""
    np.shape(ds['lnsp'][:,0,:,:])
    lnsp = ds['lnsp'][:,0,:,:].expand_dims(dim={'level':138}, axis=1)
    dims = np.shape(lnsp)
    ...
    print('Compute geopotential height for all grid points...')
    ...
    print('Interpolate data to regular vertical grid...')
